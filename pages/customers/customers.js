 (function() {
  ...
  let isSaving = false;
  const utils = window.CustomerPageUtils || {};
  const sanitizeName = utils.sanitizeName || function(value) { return (value || '').trim(); };
  const sanitizePhone = utils.sanitizePhone || function(value) { return (value || '').toString().trim(); };
  const getAllowedCountries = utils.getAllowedCountries || function() { return []; };
    ...
    try {
      ...
      clients.forEach(client => {
        ...
        row.innerHTML = `
          ...
          <td>${client.pais || '-'} </td>
          <td>${formatCreatedAt(client.fecha_creacion)}</td>
          <td>
